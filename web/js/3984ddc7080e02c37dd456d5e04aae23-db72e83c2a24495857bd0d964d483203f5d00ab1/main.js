...
let filesToLoad = [ loadFile('data/room-list.csv'), loadFile('data/user-list.csv') ]
...
async function main () {
  ...
  let lineGraphState = mergeDeep({}, graphData, {
    source: 'data/rooms/5565a1d415522ed4b3e10094.csv',
  ...
  const top20GraphData = mergeDeep({}, graphData, {
    source: 'data/top20-messages.csv',
    active: false,
    mouseOverHandler: async (data, d, i) => {
      let name = data[i].name
      let file = await fetch(`data/users/${users[name]}.json`)
